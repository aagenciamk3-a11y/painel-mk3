...
function empreendimentoDe(l){
  ...
  t=t.split(" ").map(p=>p.length>2 ? p.charAt(0).toUpperCase()+p.slice(1).toLowerCase() : p).join(" ");
  const chave=semAcento(t);
  if(RUIDO.indexOf(chave)>=0) return "";   // nome de campanha generico nao vira empreendimento
  return GRAFIA[chave] || t;
}
const semAcento = s => String(s||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim();
/* nomes de campanha que nao sao empreendimento: a mensagem cai no texto generico */
const RUIDO = ["teste","test","geral","cadastro","leads","lead","publico frio","publico quente",
  "lookalike","remarketing","interesse","trafego","conversao","captacao","novo","copia","padrao"];
/* o Meta costuma vir sem acento; aqui so arrumo a grafia, nao filtro */
const GRAFIA = { "ataide":"Ataíde", "jardim america":"Jardim América", "itapua":"Itapuã",
  "vitoria":"Vitória", "sao torquato":"São Torquato", "morada de laranjeiras":"Morada de Laranjeiras" };
...
function mensagemDe(l){
  ...
  const eu=primeiroNome((C&&C.nome)||"");          /* quem assina e o dono do portal */
  const ola = nome ? ("Oi, "+nome+"! ") : "Oi! ";
  const sou = eu ? ("Aqui é a "+eu+". ") : "";
  if(emp) return ola+sou+"Vi que você demonstrou interesse no "+emp+
    ". Posso te mandar os valores e as condições, e tirar qualquer dúvida?";
  return ola+sou+"Vi que você demonstrou interesse em um dos nossos anúncios. "+
...
function mesesComLead(){
  const m={};
  listaLeads().forEach(x=>{ const k=ymDeData(x.quando)||"0000-00"; m[k]=(m[k]||0)+1; });
  const hoje=ymDe(0);
  if(!m[hoje]) m[hoje]=0;                       /* o mes corrente sempre aparece */
...
function trafegoHTML(){
  ...
  const topo='<section class="bloco"><h2>Tráfego pago</h2>'+
    ...
    'Quanto mais rápido o contato, maior a chance de resposta.</p>'+
    (meses.length>1
      ? '<div class="tp-meses">'+meses.map(m=>
...
function cicloEtapas(){
  ...
  ETAPAS.forEach(e=>{
    const t=acharEtapa(e,ym); if(!t) return;
    /* etapa que a MK3 ainda nao marcou aparece como "a combinar",
       em vez de sumir e deixar o mes com um buraco */
    if(!t.data){
      out.push({rot:e.rot, dono:e.dono, verbo:e.verbo, inicio:null, prazo:null,
                real:null, atraso:0, estado:"adefinir", arrasto:arrasto});
      return;
    }
...
function cicloHTML(){
  ...
  if(et.length<2 || !et.some(x=>x.prazo)) return '';
  ...
  const linhas=et.map(x=>{
    const janela = !x.prazo ? "data a combinar"
                 : (x.inicio===x.prazo ? fmt(x.prazo) : (fmt(x.inicio)+" a "+fmt(x.prazo)));
    let nota="";
    if(x.estado==="adefinir")  nota="a MK3 ainda vai marcar a data";
    else if(x.estado==="ok")   nota="feito no prazo"+(x.real?", em "+fmt(x.real):"");
